...
import { GametableDotsAndBoxesConfigInterface } from './gametable-config.interface';
...
@Component({
    selector: 'pl-gametable-dots-and-boxes-drawer',
    templateUrl: './pl-gametable-dots-and-boxes-drawer.component.html',
    styleUrls: ['./pl-gametable-dots-and-boxes-drawer.component.less'],
})
export class PLGametableDotsAndBoxesDrawerComponent implements OnInit {
    @Input() activity: any;
    ...
    ngOnInit() {
        // setup form
        this.formData = PLVendorGames.dotsAndBoxes.newFormData();
    ...
    private refreshPlayerOpts() {
        ...
        const refreshedPlayers = playerOpts.map(({value}) => value);
        const nums = [1,2,3,4];
    ...
    private setPlayersFromSavedGame(gameData: GametableDotsAndBoxesConfigInterface) {
    ...
    private getGameConfig(): GametableDotsAndBoxesConfigInterface {
        return {
            gameId: Date.now(),
